...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '62ae9f18-1be5-4ef4-a3a0-60d4cfe48ac2'; // ID DEL NEGOCIO DE PRUEBA
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Negocio de Prueba';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '51234567';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenido a nuestro salón!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'prueba-notifications';
};
...
console.log('✅ config-negocio.js listo para Negocio de Prueba');
